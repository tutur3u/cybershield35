...
export async function reconcileApifyAccountHistory(dates: string[], apply = false) {
  if (process.env.APIFY_ACCOUNT_DEDICATED_TO_CS35 !== "true") throw new Error("Dedicated Apify account attribution must be configured first");
  ...
  const get = async (path: string) => {
    ...
    if (!response.ok) throw new Error(`Apify billing request failed (${response.status})`);
    return (await response.json()).data;
  };
  const account = await get("users/me");
  if (typeof account?.id !== "string" || !/^[a-zA-Z0-9_-]+$/.test(account.id)) throw new Error("Apify account identity unavailable");
  const daily = new Map<string, number>();
  for (const date of dates) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error("Invalid billing date");
    const data = await get(`users/me/usage/monthly?date=${date}`);
    for (const [day, amount] of parseApifyBillingCycle(data)) daily.set(day, amount);
  }
  ...
  for (const [day, amount] of daily) months.set(day.slice(0,7), (months.get(day.slice(0,7)) ?? 0) + amount);
  return { days: daily.size, totalUsd: [...daily.values()].reduce((s,n)=>s+n,0),
...
export async function syncAccountCosts(token: string, workspace: string, baseUrl: string) {
  ...
  for (const row of rows) {
    const response = await fetch(`${baseUrl.replace(/\/+$/, '')}/provider-costs`, {
