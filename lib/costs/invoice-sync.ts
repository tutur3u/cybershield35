...
export async function syncBillingInvoices(input: {
	...
	try {
		invoices =
			input.invoices ??
			parseBillingInvoices(process.env.CS35_BILLING_INVOICES_JSON);
	} catch {
	...
	for (const invoice of invoices) {
		try {
			const response = await (input.fetchImpl ?? fetch)(
				`${input.baseUrl.replace(/\/+$/, "")}/provider-invoices`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${input.token}`,
						"Content-Type": "application/json",
						"X-Tuturuuu-Workspace-Id": input.workspace,
					},
					body: JSON.stringify({
						...invoice,
						accountId: "cs35-dedicated",
			...
			if (!response.ok)
				return { synced, status: `invoice_upstream_${response.status}` };
			const receiptResult = z
				.object({ accepted: z.literal(true), reference: z.string() })
