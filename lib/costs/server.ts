...
export async function reconcileApifyCosts(limit = 25, apply = true) {
	...
	if (!token)
		throw new Error("APIFY_TOKEN is required for cost reconciliation");
...
export async function syncProviderCosts(
	...
	if (process.env.TUTURUUU_PROVIDER_COST_SYNC_ENABLED !== "true")
		return { synced: 0, status: "disabled" };
	if (!token || !workspace) return { synced: 0, status: "credential_missing" };
	const accountSync = await syncAccountCosts(
		token,
		workspace,
  ...
  `;
	...
	for (const row of rows) {
		...
		const status = await deliverProviderCost({
			cost,
			runId: String(row.output.runId),
...
export async function getProviderCostOverview() {
  ...
  `;
	return {
		...
		accountMonths: accountMonths.map((row) => ({
			month: row.month,
			days: row.days,
			amountUsd: Number(row.amount_usd),
			synced: row.synced,
