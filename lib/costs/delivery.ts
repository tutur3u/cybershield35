...
export async function deliverProviderCost(input: {
	...
	const response = await (input.fetchImpl ?? fetch)(
		`${input.baseUrl.replace(/\/+$/, "")}/provider-costs`,
		{
			method: "POST",
			headers: {
			...
			body: JSON.stringify({
				...
				amountUsd: input.cost.amountUsd,
				currency: "USD",
				occurredAt: input.cost.occurredAt,
				observedAt: input.cost.observedAt,
				source: "provider_api",
			}),
	...
	const receiptResult = z
		.object({ accepted: z.literal(true), externalRunId: z.string() })
		.safeParse(await response.json().catch(() => null));
	const receipt = receiptResult.success ? receiptResult.data : null;
	return receipt?.accepted === true && receipt?.externalRunId === input.runId
		? "accepted"
		: "invalid_receipt";
}
