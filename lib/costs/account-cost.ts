...
export function parseApifyBillingCycle(data: unknown) {
	if (!data || typeof data !== "object")
		throw new Error("Billing cycle unavailable");
	const cycle = data as {
		dailyServiceUsages?: unknown;
		totalUsageCreditsUsdAfterVolumeDiscount?: unknown;
	};
	if (
		!Array.isArray(cycle.dailyServiceUsages) ||
		typeof cycle.totalUsageCreditsUsdAfterVolumeDiscount !== "number" ||
		!Number.isFinite(cycle.totalUsageCreditsUsdAfterVolumeDiscount)
	)
		throw new Error("Daily billing unavailable");
	...
	for (const item of cycle.dailyServiceUsages) {
		...
		if (
			...
			throw new Error("Invalid daily billing record");
		daily.set(date, amount);
	}
	const total = [...daily.values()].reduce((sum, amount) => sum + amount, 0);
	if (
		Math.abs(total - cycle.totalUsageCreditsUsdAfterVolumeDiscount) > 0.000001
	) {
		throw new Error(
			"Daily usage does not reconcile with discounted billing total",
		);
	}
	return daily;
}
...
export function providerCostTimestamp(value: string) {
	...
	if (!Number.isFinite(timestamp.getTime()))
		throw new Error("Invalid provider cost timestamp");
	return timestamp.toISOString();
}

/** Preserve provider identity when delivering a shared daily-cost ledger. */
export function accountCostPayload(row: {
	id: string;
	provider: string;
	account_id: string;
	day: string;
	...
	return {
		...
		observedAt: providerCostTimestamp(row.observed_at),
	};
}
