...
export async function generateArticleRevision(options: {
	...
	const { output } = await generateText({
		...
		system: [
			...
			"Tiêu đề phải là một dòng độc lập, cụ thể, tự nhiên, không giật gân; tuyệt đối không nối mô tả hoặc câu mở đầu thân bài vào tiêu đề.",
			"Mô tả là một hoặc hai câu hoàn chỉnh, không lặp lại tiêu đề, không bị cắt giữa từ và không chứa ký hiệu trích dẫn.",
