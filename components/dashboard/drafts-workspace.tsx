		...
		<div className="space-y-4">
			...
			<div className="grid gap-3 xl:grid-cols-2">{items.map((draft) => (
				<article key={draft.id} className="group rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm transition hover:border-[var(--border-strong)]">
					...
					{draft.generationReason ? <p className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-[var(--accent-soft)] px-2 py-1 text-[10px] font-bold text-[var(--accent-strong)]"><ShieldCheck size={12} />{draft.generationReason === "at_risk_page" ? "Tự động: nguồn có rủi ro" : draft.generationReason === "neutral_page" ? "Tự động: nguồn trung lập" : "Tự động: nội dung tích cực từ nguồn đáng tin"}</p> : null}
