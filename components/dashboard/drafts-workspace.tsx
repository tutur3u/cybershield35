...
import { FilePenLine, LoaderCircle, Search, ShieldCheck } from "lucide-react";
	...
	return (
		<div className="space-y-4">
			...
			<div className="grid gap-3 xl:grid-cols-2">{items.map((draft) => (
				<Link key={draft.id} href={`/drafts/${draft.id}?scanId=${draft.scanJobId}`} className="group rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm transition hover:border-[var(--border-strong)]">
					...
					<p className="mt-3 line-clamp-4 whitespace-pre-wrap text-sm font-semibold leading-6 text-[var(--foreground)] group-hover:text-[var(--accent-strong)]">{draft.body}</p>
					...
					<div className="mt-4 flex items-center justify-between border-t border-[var(--border)] pt-3 text-[10px] font-semibold text-[var(--muted)]"><span>{draft.createdByDisplayName ?? "Thành viên"} · {draft.tone}</span><span className="inline-flex items-center gap-1 text-[var(--success-strong)]"><ShieldCheck size={12} /> Cần người duyệt</span></div>
				</Link>
			))}</div>
