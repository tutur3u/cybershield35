...
function PageTrustBadge({ classification }: { classification: TimelinePost["pageClassification"] }) { if (classification === "uncategorized") return null; const Icon = classification === "trusted" ? ShieldCheck : ShieldAlert; return <span className={`inline-flex h-6 items-center gap-1 rounded-md px-2 text-[11px] font-bold ${classification === "trusted" ? "bg-[var(--success-soft)] text-[var(--success-strong)]" : "bg-[var(--danger-soft)] text-[var(--danger-strong)]"}`}><Icon size={12} />{classification === "trusted" ? "Nguồn đáng tin" : "Nguồn có rủi ro"}</span>; }
