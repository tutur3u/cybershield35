	...
	return (
		<article data-evidence-id={post.id} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-soft)] transition hover:border-[var(--border-strong)]" style={{ contentVisibility: "auto", containIntrinsicSize: "260px" }}>
			<div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
				...
				<div className="flex flex-wrap items-center gap-2"><PageTrustBadge classification={post.pageClassification} />{post.triage.isPinned ? <Badge icon={Pin} label="Đội ngũ ghim" accent /> : null}<TriageBadge status={post.triage.status} /><RiskPill risk={post.riskLevel} /></div>
			</div>
			...
			<div className="mt-4 flex flex-col gap-3 border-t border-[var(--border)] pt-3 sm:flex-row sm:items-center sm:justify-between">
				...
				<div className="flex flex-wrap gap-3 text-xs font-bold"><IntentPrefetchLink href={post.href} className="inline-flex items-center gap-1 text-[var(--accent-strong)]"><Database size={14} /> Xem chi tiết</IntentPrefetchLink><button type="button" onClick={() => onDraft(post.id)} className="inline-flex items-center gap-1 text-[var(--accent-strong)]"><Sparkles size={14} /> {draftActionLabel(post.pageClassification)}</button><button type="button" onClick={() => onTriage(post.id)} className="inline-flex items-center gap-1 text-[var(--accent-strong)]"><MessageSquareText size={14} /> Xử lý</button><IntentPrefetchLink href={post.scanHref} className="text-[var(--accent-strong)]">Mở scan</IntentPrefetchLink>{post.originalPostHref ? <a href={post.originalPostHref} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[var(--accent-strong)]">Bài gốc <ExternalLink size={12} /></a> : null}</div>
			</div>
		</article>
	);
}
...
function PageTrustBadge({ classification }: { classification: TimelinePost["pageClassification"] }) { if (classification === "uncategorized") return null; const neutral = classification === "neutral"; const Icon = classification === "trusted" ? ShieldCheck : neutral ? Scale : ShieldAlert; return <span className={`inline-flex h-6 items-center gap-1 rounded-md px-2 text-[11px] font-bold ${classification === "trusted" ? "bg-[var(--success-soft)] text-[var(--success-strong)]" : neutral ? "bg-[var(--accent-soft)] text-[var(--accent-strong)]" : "bg-[var(--danger-soft)] text-[var(--danger-strong)]"}`}><Icon size={12} />{classification === "trusted" ? "Nguồn đáng tin" : neutral ? "Nguồn trung lập" : "Nguồn có rủi ro"}</span>; }
function draftActionLabel(classification: TimelinePost["pageClassification"]) { return classification === "trusted" ? "Soạn bài tích cực" : classification === "at_risk" ? "Soạn phản biện" : "Soạn phản hồi"; }
