...
function ServicePanel({ overview, schedulerReady }: { overview: OperationsOverview; schedulerReady: boolean }) {
	return <Panel><PanelHeader title="Dịch vụ & heartbeat" description="Theo dõi lịch cron đang triển khai; heartbeat của bộ lập lịch cũ được giữ để đối chiếu." action={<span className={`rounded-full px-2 py-1 text-[10px] font-bold ${schedulerReady ? "bg-[var(--success-soft)] text-[var(--success-strong)]" : "bg-[var(--warning-soft)] text-[var(--warning-strong)]"}`}>{schedulerReady ? "Cron sẵn sàng" : "Cron cần cấu hình"}</span>} /><div className="divide-y divide-[var(--divider)]">{overview.services.map((service) => <div key={service.serviceName} className="flex items-center justify-between gap-3 px-4 py-3"><div className="flex min-w-0 items-center gap-3"><span className={`size-2.5 shrink-0 rounded-full ${service.health === "healthy" ? "bg-[var(--success-strong)]" : service.health === "inactive" ? "bg-[var(--muted)]" : "bg-[var(--danger-strong)]"}`} /><div className="min-w-0"><p className="truncate text-xs font-bold text-[var(--foreground)]">{service.label}{service.health === "inactive" ? " · Lịch sử" : ""}</p><p className="mt-1 truncate text-[10px] font-semibold text-[var(--muted)]">{service.lastSeenAt ? formatDate(service.lastSeenAt) : "Chưa có heartbeat"}</p></div></div><span className="text-[10px] font-bold text-[var(--muted-strong)]">{service.ageSeconds === null ? "—" : `${formatDuration(service.ageSeconds * 1000)} trước`}</span></div>)}{overview.services.length ? null : <div className="px-4 py-6 text-xs font-semibold text-[var(--muted)]">Chưa ghi nhận heartbeat. Chạy một job để khởi tạo tín hiệu.</div>}</div></Panel>;
}
...
function operationsHealth(overview?: OperationsOverview) {
	...
	if (overview.services.every((service) => service.health === "inactive")) return { description: "Chưa nhận được heartbeat từ worker hoặc scheduler.", title: "Thiếu tín hiệu dịch vụ", tone: "warning" as const };
