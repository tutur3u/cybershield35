...
describe("provider-cost delivery", () => {
  ...
  test("requires an explicit matching receipt before marking synchronized", async () => {
    for (const body of [{}, {accepted: true, externalRunId: "other"}, {accepted: false, externalRunId: "run1"}]) {
      expect(await deliverProviderCost({...input, fetchImpl: (async () => Response.json(body)) as unknown as typeof fetch})).toBe("invalid_receipt");
    }
    expect(await deliverProviderCost({...input, fetchImpl: (async () => new Response("unavailable",{status:503})) as unknown as typeof fetch})).toBe("upstream_503");
  });
  test("never uploads preliminary costs", async () => {
    expect(await deliverProviderCost({...input, cost:{...input.cost,status:"pending"}, fetchImpl: (()=>{throw new Error("must not call")}) as unknown as typeof fetch})).toBe("unconfirmed");
  });
});
